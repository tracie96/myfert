...
import { LoadingOutlined, PhoneOutlined, MailOutlined } from "@ant-design/icons";
...
export default function PatDash() {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const hasPharmacy = response.payload?.pharmacyName !== "";
        ...
        console.log('Has watched videos:', hasWatchedVideos, hasPharmacy);
        
        setChecklistItems(prev => ({
          ...prev,
          learnVideos: hasWatchedVideos,
          pharmacy: hasPharmacy,
          intakeForms: true // Set to true when user profile is loaded
        }));
      } catch (error) {
  ...
  useEffect(() => {
    const allComplete = checklistItems.intakeForms && 
                       checklistItems.learnVideos && 
                       checklistItems.pharmacy;
    
    // Check if user has already seen the congratulations modal
    const hasSeenCongratulations = localStorage.getItem('hasSeenCongratulations');
    
    if (allComplete && !hasSeenCongratulations) {
  ...
  const handleCongratulationsOk = () => {
    setShowCongratulationsModal(false);
  };

  // Function to reset congratulations modal (for testing purposes)
  // const resetCongratulationsModal = () => {
  //   localStorage.removeItem('hasSeenCongratulations');
  //   setShowCongratulationsModal(false);
  // };
    ...
    <div>
        ...
        <div class="wrapButton">
          ...
          <div style={{
            ...
            <div style={{
              ...
              <input
                type="checkbox"
                  checked={checklistItems.pharmacy || userAuth?.obj?.pharmacyName}
                style={{
                  width: "20px",
                  height: "20px",
                  borderRadius: "6px",
      ...
      <Row gutter={16}>
        <Col xs={24} md={12} style={{ paddingBottom: "16px", paddingTop: 20 }}>
          <div>
            {/* Only show SERVICES header if not seen congratulations */}
            {!localStorage.getItem('hasSeenCongratulations') && (
              <div
                style={{
                  width: "100%",
            ...
            <div
              style={{
                padding: localStorage.getItem('hasSeenCongratulations') ? "0px 0px" : "16px 16px",
                borderRadius: localStorage.getItem('hasSeenCongratulations') ? "12px" : 12,
                borderWidth: localStorage.getItem('hasSeenCongratulations') ? "0px" : "1px",
                marginTop: localStorage.getItem('hasSeenCongratulations') ? "0" : "0",
                backgroundColor: "#fff",
                borderColor: localStorage.getItem('hasSeenCongratulations') ? "none" : "#C2E6F8",
                borderStyle: "solid",
              }}
            >
              {isMobile ? (
                ...
                <div>
                  ...
                  {showBookInfo && (
                    <div>
                      {/* Only show "Manage my appointments and plan" text if not seen congratulations */}
                      {!localStorage.getItem('hasSeenCongratulations') && (
                        <p>Manage my appointments and plan</p>
                      )}
                      
                      {/* Show new UI after congratulations modal has been seen */}
                      {localStorage.getItem('hasSeenCongratulations') ? (
                        <div
                          style={{
                            background: '#fff',
                        ...
                        /* Original Book Your Appointment UI */
                        <div
                          style={{
                            background: '#FDECEC',
                            borderRadius: 10,
                      ...
                      {/* Book Now Button - only show if not seen congratulations */}
                      {!localStorage.getItem('hasSeenCongratulations') && (
                        <button
                          style={{
                            background: '#3BA9F4',
      ...
      <Modal
        ...
        onOk={handleCongratulationsOk}
        onCancel={handleCongratulationsOk}
        footer={null}
        width={500}
        centered
        closable={false}
      ...
      >
        <div style={{ textAlign: 'center', padding: '20px' }}>
          ...
          <p style={{ 
            ...
            Great job completing the first steps!
          </p>
          <p style={{ 
            fontSize: '16px', 
            color: '#333', 
            ...
            Contact us to book your appointment with one of our care providers.
          </p>
          ...
          <Button
            type="primary"
            size="large"
            onClick={handleCongratulationsOk}
            style={{
